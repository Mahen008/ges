		...
		public function AjoutConsultation()
		{
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (!$this->Abonne_model->AjoutAbonne($dataa)) {
					$this->session->set_flashdata("message","l'Abonne n'est pas ajouté");
	
				} 
				else 
				{
					$this->session->set_flashdata("message","l'Abonne a été ajouté");
				}

				//return redirect(base_url()."abonne/ajouAbonne");
				return redirect(base_url()."abonnes/affichAbonne");
			} 
			else {
				$this->ajouAbonne();// MISEJ+HO L ERREUUR menamena
